...
def _do_evaluation(model: Module,
    ...
    file_io.dump_pickle_file(captions_pred, Path('captions_pred.pkl'))
    file_io.dump_pickle_file(captions_gt, Path('captions_gt.pkl'))
...
def _do_training(model: Module,
    ...
    data_path_validation = Path(
        settings_io['root_dirs']['data'],
        settings_io['dataset']['features_dirs']['output'],
        settings_io['dataset']['features_dirs']['validation'])
    ...
    logger_main.info('Getting valiadtion data')
    ...
    for epoch in range(settings_training['nb_epochs']):
        ...
        model.eval()
        with no_grad():
            validation_outputs = module_epoch_passing(
                data=validation_data,
                module=model,
                objective=objective,
                optimizer=None)

        objective_output_val, output_y_hat_val, \
            output_y_val, f_names_val = validation_outputs

        validation_loss = objective_output_val.mean().item()

        logger_main.info(f'Epoch: {epoch:05d} -- '
                         f'Training loss: {training_loss:>7.4f} | '
                         f'Validation loss: {validation_loss:>7.4f} | '
                         f'Best val loss: {prv_validation_loss:>7.4f} | '
        ...
        if divmod(epoch + 1,
            ...
            sampling_indices = sorted(randperm(len(output_y_hat_val))
                                      [:settings_training['nb_examples_to_sample']]
                                      .tolist())

            # Do the decoding
            _decode_outputs(*zip(*[[output_y_hat_val[i], output_y_val[i]]
                                 for i in sampling_indices]),
                            indices_object=indices_list,
                            file_names=[Path(f_names_val[i_f_name])
